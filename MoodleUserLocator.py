...
def setup():
    ...
    #checking that the url addy is correct and that we are seeing the correct title
    ...
    if driver.current_url == locators.moodle_url:
        ...
        if driver.current_url == locators.moodle_login_url:
            ...
            if driver.title == 'Dashboard' and driver.current_url == locators.moodle_dashboard_url:
                ...
                print(f'Login as "{username}" is successful and the Dashboard is present')
            else:
...
def create_new_user():
    ...
    sleep(0.25)
    driver.find_element(By.ID, 'id_alternatename').send_keys(locators.alternatename)
    sleep(0.25)
    # Using click to select the Interests drop-down
    ...
    for tags in locators.list_of_interests:
        driver.find_element(By.XPATH,'//div[3]/input').send_keys()
        sleep(0.25)
    ...
    sleep(0.25)
    print(f'New user created successfully at: {datetime.datetime.now()}')
    ...
    if driver.current_url == locators.moodle_users_main_page:
        ...
        if driver.find_element(By.ID, 'fgroup_id_email_grp') and driver.find_element(By.NAME, 'email'):
            ...
            if driver.find_element(By.XPATH,f'//td[contains(.,"{locators.email}")]'):
                print('For test scenario: check user created ---- successful, test passed.')
...
#create user
create_new_user()
#check to ensure user was created
check_user_created()
